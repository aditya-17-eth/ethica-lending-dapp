...
npm run compile
```
Compiles Solidity contracts and generates artifacts in the `artifacts/` directory.

#### Run Contract Tests
```bash
npm run test:contracts
```
Runs Hardhat tests for smart contracts located in the `test/` directory.

#### Start Local Blockchain Node
```bash
npm run node
```
Starts a local Hardhat node for development and testing.
...
```
Requires `.env` configuration with `SEPOLIA_RPC_URL` and `PRIVATE_KEY`.
...
npm start
```
Starts the React development server at `http://localhost:3000` with hot reloading enabled.
...
npm test
```
Runs Jest tests for React components in watch mode.

#### Run All Tests
```bash
npm test -- --watchAll=false
```
Runs all tests once without watch mode (useful for CI/CD).
...
3. **Update Frontend Config** (if needed):
   Update contract addresses in your environment or configuration files.
...
5. **Connect MetaMask**:
   - Add local network (RPC: `http://localhost:8545`, Chain ID: `31337`)
...
- **Utils**: Place pure functions in utility files
- **Styling**: Use TailwindCSS utility classes; avoid inline styles
- **Animations**: Use predefined variants from `utils/animations.js`
...
2. Import and add route in `src/App.js`:
   ...
   ```
3. Add navigation link in `src/utils/navigation.js`
4. Update Navbar to include the new link (if needed)
